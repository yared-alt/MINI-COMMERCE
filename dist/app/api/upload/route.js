...
export async function POST(request) {
    try {
        ...
        try {
            ...
            const frontresult = await new Promise((resolve, rejects) => {
        ...
        // return;
        try {
            const otherImagesResults = await Promise.all(otherImages.map(async (file) => {
                const buffer = Buffer.from(await file.arrayBuffer());
                return new Promise((resolve, reject) => {
                    const uploadStream = cloudinary.uploader.upload_stream({ folder: "mini-commerce" }, (error, result) => {
                        if (error)
                            reject(error);
                        else
                            resolve(result);
