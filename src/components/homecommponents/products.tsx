  ...
  return (
    <div className={`${maxw}`}>
      <div className="swiper-container">
        <div className="swiper-wrapper  border-2 border-black">
          {
            products && products.map((pro) => (
              <div key={pro._id} className="swiper-slide  w-[100%] border-2  border-black sm:w-1/2 md:w-1/3 lg:w-1/4">
                <div className="product__card ml-7 h-[459px] min-w-36   max-w-64 border-2 border-black  ">
                  <div className=" border-2 pb-4 h-auto  bg-slate-100">
                    <Link href={`/product/${pro._id}`}>
                      <div className="product__image overflow-hidden">
                        <Image src={`https://res.cloudinary.com/dr1ejpdn8/image/upload/v1741885737/${pro.FrontImage}`} className="block  brightness-100 hover:brightness-75 hover:transition-all duration-500 hover:scale-105 w-[260px] h-[179px] rounded-md mx-auto  " width="800" height="800" alt="" />
                      </div>
                    ...
                    <div className="product__color mt-3 max-w-[80%] mx-auto">
                      <div className=" grid gap-y-2 grid-cols-[repeat(auto-fit,minmax(30px,1fr))]">
                        ...
                        <Link key={i} href={`product/${pro._id}?color=${pro.ProductColors[i]}`}>
                          <input type="button" className="w-auto h-6 border border-black px-2 py-1 inline-flex align-middle cursor-pointer selected" value={color}></input>
                        </Link>
